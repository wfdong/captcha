...
def application(environ, start_response):
    ...
    if Num==None:
        ...
        Count = float(mc.get('731'+config._SUCCESSCOUNT)) / float(mc.get('731'+config._CALLEDCOUNT))
        if CalledCount>=10000:
            Count = float(SucCount) / float(CalledCount)
        Base = float(mc.get('731'+config._SUCCESSCOUNT)) / float(mc.get('731'+config._CALLEDCOUNT)) * float(0.4)
        Base2 = float(mc.get('731'+config._SUCCESSCOUNT)) / float(mc.get('731'+config._CALLEDCOUNT)) * float(0.5)
        if Count<Base2:
            needEvidence = 'True'
        #print 'Count:'
        #print Count
    ...
    try:
        ...
        params = { "username" : userN, "myfile" : myFile, "needEvidence" : needEvidence }
        #params = { "username" : userN, "myfile" : myFile}
        data = opener.open("http://127.0.0.1:8009/distribute", params, timeout=55)
        content = data.read()
        print(content)
