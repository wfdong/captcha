...
def sync():
    try:
        ...
        if timecount>=3600:
            ...
            ratiocur.execute("select ratio from dynamicratio where userid=1007 and date=(SELECT  now() - interval (TIME_TO_SEC(now()) mod 900) second from dual)")
            ...
            for ratiorow in ratiorows:
                ...
                if str(ratiorow[2])!='NULL' and str(ratiorow[2])!='None':
                    if (float(ratiorow[2])!=float(0)) and (float(ratiorow[2])<float(standard)*0.6):
